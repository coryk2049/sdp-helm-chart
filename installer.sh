...
if helm init --tiller-namespace $DEPLOYMENT_NAME-tiller --tiller-image gcr.io/kubernetes-helm/tiller:v2.13.0 --service-account tiller --wait > /dev/null; then
...
# jenkins_home
if oc create -f ./openshift/pvc-jenkins.yaml > /dev/null; then
